...
def povezava_gorovje(gorovje):
    ...
    # funkcija pridobi html iz url-ja
    vsebina = requests.get(url_indeks, timeout=10)
    # preverjamo ali nam bo strežnik vrnil napako
    vsebina.raise_for_status() 

    # string razčleni v html, to mu povemo s "html.parser"
    soup = BeautifulSoup(vsebina.text, "html.parser") 

    # najde vse linke na spletni strani (ki so v indeksni tabeli)
    for a in soup.find_all("a", href=True): 
        href = a["href"]

        # če povezava, do izbranega gorovja obstaja nam jo funkcija vrne
        if gorovje.lower() in href.lower(): 
...
def prenos_html_strani(url):
    ...
    for i in range (3): # funkcija 3 poskuša pridobiti podatke
        try:
            ...
            # če je bilo pridobivanje podatkov uspešno
